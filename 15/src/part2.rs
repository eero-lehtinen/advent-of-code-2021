...
fn expand_x5(nodes: &mut Vec<Vec<i32>>) {
	for line in nodes.iter_mut() {
		...
		for x in 1..5 {
			line.extend(
				line_clone
					.iter()
					.map(|n| if n + x > 9 { (n + x) % 10 + 1 } else { n + x }),
			);
		}
	}
	...
	for y in 1..5 {
		for line in nodes_clone.iter() {
			nodes.push(
				line.iter()
					.map(|n| if n + y > 9 { (n + y) % 10 + 1 } else { n + y })
					.collect::<Vec<_>>(),
			);
		}
	}
...
fn main() {
	...
	let mut nodes = contents.lines().map(parse_num_line).collect::<Vec<_>>();
